...
class DocumentedTypeDef extends DocumentedItem {
	...
	registerMetaInfo(data) {
		...
		if (data.properties) {
			if (data.properties.length > 0) for (let i = 0; i < data.properties.length; i++) data.properties[i] = new DocumentedParam(this, data.properties[i]);
			else data.properties = undefined;
		}
