...
class DocumentedClass extends DocumentedItem {
	...
	add(item) {
		if (item instanceof DocumentedConstructor) {
			...
			if (this.methods.has(prefix + item.directData.name)) throw new Error(`Doc ${this.directData.name} already has method ${item.directData.name}`);
			this.methods.set(prefix + item.directData.name, item);
		} else if (item instanceof DocumentedMember) {
			if (this.props.has(item.directData.name)) throw new Error(`Doc ${this.directData.name} already has prop ${item.directData.name}`);
			this.props.set(item.directData.name, item);
		} else if (item instanceof DocumentedEvent) {
			if (this.events.has(item.directData.name)) throw new Error(`Doc ${this.directData.name} already has event ${item.directData.name}`);
	...
	serializer() {
		return {
			...
			construct: this.construct ? this.construct.toJSON() : undefined,
			props: this.props.size > 0 ? Array.from(this.props.values()).map(param => param.toJSON()) : undefined,
			methods: this.methods.size > 0 ? Array.from(this.methods.values()).map(method => method.toJSON()) : undefined,
			events: this.events.size > 0 ? Array.from(this.events.values()).map(event => event.toJSON()) : undefined,
